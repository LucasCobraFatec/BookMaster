...
export function useRollTables(campaignId: string) {
  ...
  const createRollTable = async (targetCampaignId: string, name: string, formula = '1d20') => { const table: RollTable = { id: crypto.randomUUID(), campaignId: targetCampaignId, name, formula, results: [], isDraft: true }; await db.rollTables.add(table); return table; };
  const updateRollTable = async (id: string, updates: Partial<RollTable>): Promise<void> => { await db.rollTables.update(id, { ...updates, isDraft: false }); };
