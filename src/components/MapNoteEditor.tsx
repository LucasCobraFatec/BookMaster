...
export function MapNoteEditor({ content, notes, onChange }: { content: string; notes: NoteEntity[]; onChange: (value: string) => void }) {
  ...
  const uploadImage = (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => typeof reader.result === 'string' && update({ ...data, image: reader.result }); reader.readAsDataURL(file); };
