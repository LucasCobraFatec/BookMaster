...
export function DungeonNoteEditor({ content, onChange }: { content: string; onChange: (content: string) => void }) {
  ...
  return <div className="space-y-6">
    <section><label className="mb-2 block text-sm font-semibold text-sky-300">Imagem da masmorra</label>{data.image ? <div className="group relative overflow-hidden rounded-xl border border-zinc-700 bg-zinc-950"><img src={data.image} alt="Prévia da masmorra" className="max-h-80 w-full object-contain" /><button type="button" onClick={() => update({ ...data, image: '' })} aria-label="Remover imagem" className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-lg bg-zinc-950/85 text-zinc-300 opacity-0 group-hover:opacity-100"><X className="h-4 w-4" /></button></div> : <label className="flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-zinc-700 bg-zinc-900/30 text-zinc-500 hover:border-sky-500/60 hover:text-sky-400"><ImagePlus className="mb-2 h-7 w-7" /><span className="text-xs font-semibold">Selecionar imagem</span><span className="mt-1 text-[10px]">PNG, JPG, WEBP ou GIF</span><input type="file" accept="image/*" onChange={uploadImage} className="hidden" /></label>}</section>
