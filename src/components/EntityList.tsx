...
export function EntityList({ characters, onSelect }: { characters: CharacterEntity[]; onSelect: (character: CharacterEntity) => void }) {
  ...
  return <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{characters.map((character) => <EntityCard key={character.id} character={character} onSelect={() => onSelect(character)} />)}</div>;
}
